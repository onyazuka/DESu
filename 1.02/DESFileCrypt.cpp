...
void File_Crypter::run_des(char* buffer, int offset)
{
	...
	memcpy(buffer + offset, &res, BLOCKSIZE);
}
...
int File_Crypter::run_mt()
{
	...
	while (ifs.gcount() != 0)
	{
		...
		for (int i = 0; i < to_read; i += BLOCKSIZE)
		{
			thread_pool.wait_do_task(std::bind(&File_Crypter::run_des, this, buffer, i));
			//std::cout << i << std::endl;
		}
