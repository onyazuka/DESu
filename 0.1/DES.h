...
const int EXPANDED_HALF_BLOCK_SIZE = 48;

typedef unsigned char uchar;
typedef unsigned long long ull;
typedef std::array<std::array<int, 16>, 8> sub_table;
...
static const std::array<int, 32> f_final_permutation_array
{
...
/*
	Function expanding block of data from 32 bits to 48bits(as key is)
	Products new block, and we want to pass owning of it, so we are using std::unique_ptr
*/
std::unique_ptr<uint64_t> expanding(uchar* block)
{

	// 64bits = 8bytes, can be represented as ull
	uint32_t* block_ull = reinterpret_cast<uint32_t*>(block);
	uint32_t block_ull_copy = *block_ull;
	std::unique_ptr<uint64_t> res = std::make_unique<uint64_t>();
	for (int i = 0; i < EXPANDED_HALF_BLOCK_SIZE; ++i)
	{
		bool temp = (block_ull_copy & ((uint64_t)1 << (expanding_array[EXPANDED_HALF_BLOCK_SIZE - 1 - i]) - 1))
			>> (expanding_array[EXPANDED_HALF_BLOCK_SIZE - 1 - i] - 1);
...
*/
void append_key_to_odd(uchar* key)
{
	uint64_t new_key = 0;
	uint64_t cur_key = *reinterpret_cast<uint64_t*>(key);
	for (int i = 0; i < 8; ++i)
	{
