...
//function that asynchronious writes into output stream ofs data from buffer(size bytes)
bool writer(std::ofstream& ofs, char* buffer, int size)
{
...
int File_Crypter::run_mt()
{
	...
	// using two buffers so we don't block on write, but using reserve buffer
	// so run_des alternates buffers, and writing in output file is runned asynchronously
	char* main_buffer = new char[BUFSIZE];
	char* reserve_buffer = new char[BUFSIZE];
	char* active_buffer = main_buffer;
	char* passive_buffer = reserve_buffer;
	std::packaged_task<bool()> async_writer;
	std::future<bool> answer_from_writer;
	bool waiting_first_time = true;
	ifs.read(active_buffer, BUFSIZE);
	uint64_t res;
	while (ifs.gcount() != 0)
	{
		// alignning data to 64 bits
		int to_align = (BLOCKSIZE - ifs.gcount() % BLOCKSIZE) == BLOCKSIZE ? 0 : (BLOCKSIZE - ifs.gcount() % BLOCKSIZE);
		int to_read = ifs.gcount() + to_align;
		memset(active_buffer + ifs.gcount(), 0, to_align);
		// processing blocks
		int offset = 0;
		for (int i = 0; i < thread_pool.size() - 1; ++i)
		{
			thread_pool.wait_do_task(std::bind(&File_Crypter::run_des, this, active_buffer + offset, to_read / BLOCKSIZE / thread_pool.size()));
			offset += to_read / thread_pool.size();
		}
		// processing last portion of blocks
		thread_pool.wait_do_task(std::bind(&File_Crypter::run_des, this, active_buffer + offset, (to_read - offset) / BLOCKSIZE));

		//running asynchronous writer
		async_writer = std::packaged_task<bool()>{ std::bind(writer, std::ref(ofs), active_buffer, to_read) };
		answer_from_writer = async_writer.get_future();
		async_writer();
		//swapping buffers
		char* active_temp = active_buffer;
		active_buffer = reserve_buffer;
		reserve_buffer = active_temp;
		// if swapping first time, we don't need to wait answer
		if (!waiting_first_time)
		{
			answer_from_writer.get();
			waiting_first_time = false;
		}
		ifs.read(active_buffer, BUFSIZE);
	}
