...
private:
    ...
    void fill_6bits_blocks(uint64_t& number, std::array<uchar, 8>& blocks);
...
UINT rol(UINT num, int val, int digits = -1)
{
...
*/
template<typename UINT>
UINT ror(UINT num, int val, int digits = -1)
{
...
void transformation(uint64_t& block, int size, T& transform_array)
{
	...
	{
#ifdef _MSC_VER		//much faster for msvc
		temp = (block_copy & lshift_table[transform_array[size - 1 - i]]);
		if (temp)
		{
			res |= lshift_table[i];
		}
#else				//much faster for gcc
		temp = (block_copy & ((uint64_t)1 << transform_array[size - 1 - i]));
		if (temp)
		{
			res |= ((uint64_t)1 << i);
		}
