...
constexpr UINT rol(UINT num, int val, int digits = -1)
{
	...
	UINT mask = (UINT)pow(2, digits) - 1;
	if (digits == -1)
	{
		digits = sizeof(UINT) * CHAR_BIT;
		mask = (UINT)pow(2, digits) - 1;
	}
	num &= mask;
...
*/
template<typename T>
std::unique_ptr<uint64_t> transformation(uint64_t* block, int size, T& select_array)
{
...
{
	std::unique_ptr<uint64_t> expanded_r = transformation(reinterpret_cast<uint64_t*>(ri_minus1), EXPANDED_HALF_BLOCK_SIZE, expanding_array); // now 48 bits
	*expanded_r = modulo2_addition(*expanded_r, *ki);
	...
	permutation(reinterpret_cast<uint64_t*>(&res), HALF_BLOCK_SIZE, f_final_permutation_array);
	return (uint32_t)(res);
}

void update_key(uint64_t* key, int round, bool encrypt)
{
	...
	uint32_t C = (*key & ((uint64_t)0xfffffff << 28)) >> 28;
	...
	std::unique_ptr<uint64_t> res = transformation(&CD, FINAL_KEY_SIZE, key_selection_array);
	*key = *res;
...
void decrypt(uint64_t* block, uint64_t* key)
{
