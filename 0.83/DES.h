...
static std::array<uchar, BLOCK_SIZE> lshift_table;

static std::array<ushort, BLOCK_SIZE> byte_array;
static std::array<uchar, BLOCK_SIZE> byte_offset_array;

static const std::array<uchar, BLOCK_SIZE> byte_table;

void init_tables()
{
	for (int i = 0; i < BLOCK_SIZE; ++i)
	{
		//lshift_table[i] = (uchar)1 << (i % 8);
		byte_array[i] = ((i / 8) << 8) + ((uchar)1 << (i % 8));
		//byte_offset_array[i] = i % 8;
	}
}
...
void transformation(uint64_t& block, int size, T& transform_array)
{
	...
	uint64_t res = 0;
	uchar* pblock_copy = reinterpret_cast<uchar*>(&block_copy);
	uchar* pres = reinterpret_cast<uchar*>(&res);
	bool temp = true;
	for (int i = 0; i < size; ++i)
	{
		int index = transform_array[size - 1 - i];
		temp = *(pblock_copy + (byte_array[index] >> 8)) & (uchar)byte_array[index];
		//temp = (block_copy & lshift_table[transform_array[size - 1 - i]]);
		//std::cout << temp;
		if (temp)
		{
			*(pres + (byte_array[i] >> 8)) |= (uchar)byte_array[i];
			//res |= lshift_table[i];
		}
	}
	block = res;
